//*****************************************************************************
// This StellarisWare example modified to work with Procyon
...
main(void)
{
    ...
    UARTprintf("  Size: 16MB (8Meg x 16bits)\n\n");
    ...
    // refresh counter.
    //
    //EPIConfigSDRAMSet(EPI0_BASE, EPI_SDRAM_CORE_FREQ_15_30 | EPI_SDRAM_FULL_POWER | EPI_SDRAM_SIZE_128MBIT, 1024);
    EPIConfigSDRAMSet(EPI0_BASE, EPI_SDRAM_CORE_FREQ_30_50  | EPI_SDRAM_FULL_POWER | EPI_SDRAM_SIZE_128MBIT, 1024);
    ...
    UARTprintf("     Mem[0x603F.FFFE] = 0x%4x\n",
               g_pusEPISdram[SDRAM_END_ADDRESS - 1]);
    UARTprintf("     %x = 0x%4x\n\n",
               SDRAM_END_ADDRESS, g_pusEPISdram[SDRAM_END_ADDRESS]);
	...
	UARTprintf("Quick memory check\n");



	unsigned long k = 0;
	unsigned long k2 = 0;
	while(k<SDRAM_END_ADDRESS)
	{
		g_pusEPISdram[k] = 0x5555;
	...
	while(k<SDRAM_END_ADDRESS)
	{
		if(g_pusEPISdram[k] != 0x5555)UARTprintf("Mismatch at %x\n", k);
		k++;
	}
